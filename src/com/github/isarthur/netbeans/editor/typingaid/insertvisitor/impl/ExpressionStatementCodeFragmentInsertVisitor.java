 ...
 */
public class ExpressionStatementCodeFragmentInsertVisitor extends AbstractCodeFragmentInsertVisitor {

    @Override
    protected Tree getNewTree(CodeFragment codeFragment, Tree tree, CodeCompletionRequest request) {
        ExpressionStatementTree originalTree = (ExpressionStatementTree) getOriginalTree(codeFragment, request);
        ...
        if (!tokenSequence.moveNext() && tokenSequence.token().id() != JavaTokenId.IDENTIFIER) {
            return null;
        }
        Trees trees = workingCopy.getTrees();
        TypeMirror typeMirror = trees.getTypeMirror(treeUtilities.pathFor(tokenSequence.offset() + 1));
        return JavaSourceMaker.makeVariableTree(
                JavaSourceMaker.makeModifiersTree(Collections.emptySet(), request),
                tree.toString(),
                JavaSourceMaker.makeTypeTree(typeMirror, request),
                null,
                request);
    }
}
